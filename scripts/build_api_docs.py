...
# v0.5 management and security operations.
schemas['Login']['properties']['code']={'type':'string','description':'Authenticator or unused recovery code when MFA is enabled.'}
for key in ['memory_mb','cpu_millis','disk_mb']:schemas['ApplicationCreate']['properties'][key]=I
schemas['ApplicationCreate']['properties']['version']=S
endpoint('/api/v5/apps/{id}/console','post','Stream application command output','Workspace','Owned application. Runs a container command for at most 25 seconds. Newline-delimited JSON output, bounded to 2 MiB. Disconnect cancels the command.',obj({'command':S},['command']),{'command':'printf hello'},response=S,media='application/x-ndjson')
endpoint('/api/v5/network-range','post','Preview IP and CIDR ranges','Workspace','Returns canonical first and last addresses or validation errors for comma/newline-separated inputs.',obj({'value':S},['value']),{'value':'10.10.10.0/24'},response=array(O))
endpoint('/api/v5/apps/{id}/git','post','Workspace Git tools','Workspace','Owned application; fixed status, history, diff or init operations. Full Git is available inside the IDE.',O,{'operation':'status'})
endpoint('/api/v5/apps/{id}/services','get','Transfer service settings','Workspace','Owned application. Returns SFTP and FTPS status, username, directory and IP restrictions.')
endpoint('/api/v5/apps/{id}/services','post','Configure SFTP and FTPS','Workspace','Owned application. FTPS requires explicit TLS. A new or rotated password is returned once. Existing sessions close when access is disabled or credentials rotate.',obj({'sftp':B,'ftps':B,'rotate':B,'allowed_ips':IDS},['sftp','ftps']),{'sftp':True,'ftps':False,'rotate':False,'allowed_ips':['203.0.113.0/24']})
endpoint('/api/v5/users/{id}/budget','get','Read account resource budget','Resources','Own account or administrator. Returns budget, allocated, remaining, and per-application enforcement.')
endpoint('/api/v5/users/{id}/budget','post','Set account resource budget','Administration','Administrator only. Cannot set below existing allocations. Disk covers application workspaces, not mail, databases or container images.',obj({'memory_mb':I,'cpu_millis':I,'disk_mb':I},['memory_mb','cpu_millis','disk_mb']),{'memory_mb':2048,'cpu_millis':2000,'disk_mb':4096})
endpoint('/api/v5/apps/{id}/limits','post','Allocate application resources','Resources','Owned running application. Allocations must fit account budget. Workspace migration/growth briefly stops its app and IDE; disk shrinking is rejected.',obj({'memory_mb':I,'cpu_millis':I,'disk_mb':I},['memory_mb','cpu_millis','disk_mb']),{'memory_mb':512,'cpu_millis':1000,'disk_mb':1024})
endpoint('/api/v5/apps/{id}/backup-import','post','Upload a cgp archive','Backups','Owned application. begin supplies size and returns upload; chunk supplies upload, offset and base64 data (at most 196608 decoded bytes); finish validates ownership and registers the archive; cancel removes a partial upload. Limit 2 GiB and two active uploads. Restore is a separate v2 job.',O,{'operation':'begin','size':1048576})
endpoint('/api/v5/protection/{id}','get','Website protection settings','Security','Owned domain. Provider secret is omitted; secret_set indicates an existing secret.')
endpoint('/api/v5/protection/{id}','post','Configure website protections','Security','Owned domain. WAF off/detect/enforce; crawlers allow/block_ai/block_all; CAPTCHA off/local/turnstile/hcaptcha/recaptcha. Blank provider secret preserves current. CAPTCHA gates site pages and may affect APIs or webhooks. Rate controls cannot absorb upstream network saturation.',O,{'requests_per_second':20,'connections':30,'waf':'detect','crawlers':'allow','captcha':'off','site_key':'','secret':'','sitemap':True,'sitemap_paths':['/']})
endpoint('/api/v5/protection/{id}/traffic','get','Recent traffic summary','Security','Owned domain. Aggregates most recent 2000 logged requests, not a fixed time window. Unusual flags are heuristics, not proof of an attack.')
endpoint('/api/v5/mfa','get','Own MFA status','Authentication','Includes count of remaining recovery codes.')
endpoint('/api/v5/mfa/enroll','post','Begin authenticator enrollment','Authentication','Browser session, CSRF and current password. Returns secret and otpauth URL; expires in ten minutes.',obj({'password':S},['password']),session=True)
endpoint('/api/v5/mfa/confirm','post','Confirm authenticator enrollment','Authentication','Browser session and CSRF. Activates a valid six-digit code, revokes other sessions and returns one-time recovery codes.',obj({'code':S},['code']),session=True)
endpoint('/api/v5/mfa/disable','post','Disable own MFA','Authentication','Browser session and CSRF. Requires current password and a valid authenticator/recovery code.',obj({'password':S,'code':S},['password','code']),session=True)
endpoint('/api/v5/mail','get','Mail hosting status','Mail','Tenants see their domains and mailbox metadata; administrators see all. No password hashes are returned. See bundled mail guide for DNS, TLS and provider requirements.')
endpoint('/api/v5/mail/server','post','Configure mail server','Administration','Administrator only. Set this VPS hostname, matching certificate domain ID, and public port state. Empty certificate_domain selects the development self-signed certificate.',obj({'hostname':S,'certificate_domain':S,'enabled':B},['hostname','enabled']),{'hostname':'mail.example.com','certificate_domain':'DOMAIN_ID','enabled':True})
endpoint('/api/v5/mail/domains/{id}','post','Enable domain mail','Mail','Owned domain or administrator. Enable generates DKIM keys; disable requires all its mailboxes to be disabled.',obj({'enabled':B},['enabled']),{'enabled':True})
endpoint('/api/v5/mail/mailboxes','post','Create or update mailbox','Mail','Owned mail-enabled domain. Omit id to create; include id to update. Blank password on update preserves current. Quota 64–102400 MiB, maximum 30 mailboxes per tenant. Disable preserves messages.',obj({'id':S,'domain_id':S,'local':S,'password':S,'quota_mb':I,'enabled':B},['domain_id','local','quota_mb','enabled']),{'domain_id':'DOMAIN_ID','local':'hello','password':'PRIVATE_PASSWORD','quota_mb':512,'enabled':True})
...
lines.extend(['','## Resource action payloads','','All use `POST /api/resource/{id}/{action}`. Empty requests still send `{}`.','', '| Resource / action | JSON body | Result / behavior |','| --- | --- | --- |','| apps / start, stop, restart | `{}` | Start/stop/restart the owned container |','| apps / inspect, logs | `{}` | Captured inspect/log output |','| apps / terminal | `{"command":"id"}` | UID 1000 command, 25-second timeout |','| apps / files | `{}` | Workspace file listing, depth at most 3 |','| apps / read | `{"path":"index.html"}` | Text in `output`; workspace-relative path only |','| apps / write | `{"path":"index.html","content":"Hello"}` | Write bounded text inside workspace |','| databases / access | `{"allowed_ips":["203.0.113.10"]}` | Replace IP/CIDR database remote allowlist (MySQL IPv6 networks require individual addresses) |','| schedules / status | `{}` | Next run and bounded execution history |','| backups / restore | `{"confirm":"RESTORE"}` | Legacy workspace snapshot restore |','| domains / tls | `{"email":"admin@example.com","validation":"http","agree_tos":true}` | Legacy synchronous HTTP certificate request; prefer v2 job |'])
